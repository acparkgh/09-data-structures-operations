...
const airportCode = function (message) {
  return (
    message.toUpperCase().slice(0, 3)
  )
}

for (const flight of flightsSplit) {
  const [type, from, to, time] = flight.split(";");
  const output = `${ type.startsWith("_Delayed") ? "😢" : "" }${ type.replace(/_/g, "") } from ${ airportCode(from) } to ${ airportCode(to) } (${ time.replace(":", "h") })`.padStart(42);
  console.log(output);
};